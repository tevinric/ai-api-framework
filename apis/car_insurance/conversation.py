...
from apis.car_insurance.references import (
    GPT4O_DEPLOYMENT, CAR_INSURANCE_TOOLS, CAR_INSURANCE_BOT_CREDIT_COST, 
    OFF_TOPIC_RESPONSE, CAR_MAKES, CAR_MODELS_BY_MAKE
)
from apis.car_insurance.functions import (
    create_api_response, ensure_complete_data, is_off_topic_query, extract_info_from_message,
    process_tool_calls, format_conversation_for_openai, get_valid_models_for_make, 
    get_conversation_history, save_conversation_history, delete_conversation_history,
    validate_make, get_next_question_options
)
...
def car_insurance_chat_route():
    """
    Car insurance quote chatbot API endpoint
    ---
    tags:
      - Car Insurance Bot
    parameters:
        ...
        schema:
          ...
          properties:
            ...
            next_question_options:
              type: object
              description: Options for the next question in the conversation
            prompt_tokens:
              type: integer
    ...
    # Deduct balance based on car insurance bot credit cost
    success, result = BalanceService.check_and_deduct_balance(g.user_id, endpoint_id, CAR_INSURANCE_BOT_CREDIT_COST)
    if not success:
    ...
    try:
        ...
        if conversation_id:
            ...
            if is_off_topic_query(user_message) and not conversation.get("quote_complete", False):
                ...
                if "make" in conversation.get("extraction_data", {}) and conversation["extraction_data"]["make"]:
                    valid_models = get_valid_models_for_make(conversation["extraction_data"]["make"])
                
                # Get next question options
                next_question_options = get_next_question_options(conversation)
                
                # Return the off-topic response with complete extraction data
                return create_api_response({
                    "conversation_id": conversation_id,
                    "assistant_message": OFF_TOPIC_RESPONSE,
                    "extraction_data": ensure_complete_data(conversation.get("extraction_data", {})),
                    "is_off_topic": True,
            ...
            if is_off_topic_query(user_message):
                ...
                initial_extraction = temp_conversation["extraction_data"]
                ...
                return create_api_response({
                    "conversation_id": conversation_id,
                    "assistant_message": OFF_TOPIC_RESPONSE,
                    "extraction_data": ensure_complete_data(initial_extraction),
                    "is_off_topic": True,
                    "is_quote_complete": False,
                    "valid_models": valid_models,
                    "next_question_options": next_question_options,
        ...
        if assistant_response.tool_calls:
            ...
            for result in tool_results:
                ...
                if function_name == "process_quote":
        ...
        # Get options for the next question
        next_question_options = get_next_question_options(conversation)
...
def delete_car_insurance_chat_route():
    """
    Delete a car insurance quote conversation
    ---
    tags:
      - Car Insurance Bot
    parameters:
...
def register_car_insurance_routes(app):
