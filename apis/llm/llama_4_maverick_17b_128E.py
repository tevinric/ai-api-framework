...
def llama_4_maverick_17b_128E_route():
    ...
    # Validate max_tokens
    if not isinstance(max_tokens, int) or max_tokens < 1 or max_tokens > 120000:
        return create_api_response({
            "response": "400",
            "message": "max_tokens must be an integer between 1 and 120000 for Llama 4 Maverick model"
        }, 400)
    ...
    try:
        ...
        if file_ids and len(file_ids) > 0:
            ...
            if len(file_ids) > 50:  # Reasonable limit for Llama 4 Maverick 17B 128E Instruct FP8 (128k context)
                return create_api_response({
                    "response": "400",
                    "message": "Too many files. Llama 4 Maverick 17B 128E Instruct FP8 can process a maximum of 50 image files per request."
                }, 400)
...
def register_llm_llama_4_maverick_17b_128E(app):
    ...
    app.route('/llm/llama-4-maverick-17b-128e', methods=['POST'])(
        track_usage(
            api_logger(
                check_endpoint_access(
                    check_balance(llama_4_maverick_17b_128E_route)
                )
            )
        )
    )