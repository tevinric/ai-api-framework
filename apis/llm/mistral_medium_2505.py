# apis/llm/mistral_medium_2505.py
from flask import jsonify, request, g, make_response
from apis.utils.tokenService import TokenService
from apis.utils.databaseService import DatabaseService
from datetime import datetime
from apis.utils.llmServices import mistral_medium_2505_service # Import the multimodal service function
...
def mistral_medium_2505_route():
    """
    Consumes 2 AI credits per call
    
    Mistral Medium 2505 LLM model for multimodal content generation.
        ...
        schema:
          ...
          properties:
            ...
            top_p:
              ...
              maximum: 1
              default: 0.1
              description: Controls diversity via nucleus sampling (0=focused, 1=diverse)
            file_ids:
              type: array
              items:
                type: string
    ...
    # Extract parameters with updated defaults
    system_prompt = data.get('system_prompt', 'You are a helpful AI assistant')
    user_input = data.get('user_input', '')
    temperature = float(data.get('temperature', 0.8))  # Updated default to 0.8
    max_tokens = int(data.get('max_tokens', 2048))  # Keeping default as 2048
    top_p = float(data.get('top_p', 0.1))  # New parameter with default 0.1
    ...
    try:
        # Log API usage
        logger.info(f"Mistral Medium 2505 API called by user: {user_id}")
        ...
        if file_ids and len(file_ids) > 0:
            ...
            if len(file_ids) > 20:  # Reasonable limit for Mistral Medium 2505
                return create_api_response({
                    "response": "400",
                    "message": "Too many files. Mistral Medium 2505 can process a maximum of 20 image files per request."
                }, 400)
        else:
            logger.info("Text-only request (using multimodal service)")
        
        # Always use the multimodal service - it handles both scenarios
        service_response = mistral_medium_2505_service(
            system_prompt=enhanced_system_prompt,  # Use enhanced prompt with context
            user_input=user_input,
            temperature=temperature,
            max_tokens=max_tokens,
            top_p=top_p,  # Pass the new parameter
            file_ids=file_ids,  # Will be empty list for text-only requests
            user_id=user_id
        )
        
        if not service_response["success"]:
            logger.error(f"Mistral Medium 2505 API error: {service_response['error']}")
...
def register_llm_mistral_medium_2505(app):
    ...
    app.route('/llm/mistral-medium-2505', methods=['POST'])(track_usage(api_logger(check_endpoint_access(check_balance(mistral_medium_2505_route)))))