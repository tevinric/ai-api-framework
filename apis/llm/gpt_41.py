...
from apis.utils.llmServices import gpt41_service # Import the multimodal service function
...
def gpt41_route():
    """
    Consumes 2.5 AI credits per call
    
    OpenAI GPT-4.1 LLM model for advanced text completion and content generation.
    ...
    try:
        # Log API usage
        logger.info(f"GPT-4.1 API called by user: {user_id}")
        ...
        if file_ids and len(file_ids) > 0:
            ...
            if len(file_ids) > 25:  # Higher limit for GPT-4.1
                return create_api_response({
                    "response": "400",
                    "message": "Too many files. GPT-4.1 can process a maximum of 25 image files per request."
                }, 400)
        ...
        service_response = gpt41_service(
        ...
        if not service_response["success"]:
            logger.error(f"GPT-4.1 API error: {service_response['error']}")
...
def register_llm_gpt_41(app):
    ...
    app.route('/llm/gpt-4.1', methods=['POST'])(track_usage(api_logger(check_endpoint_access(check_balance(gpt41_route)))))